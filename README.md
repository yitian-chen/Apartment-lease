...
- `web-app`: 用户移动端后端接口。支持手机验证码登录、房源检索、浏览记录、预约看房、个人租约查询及即时通讯聊天。
...
- **即时通讯:** 基于 WebSocket 的实时聊天，支持跨实例消息分发（Redis Pub/Sub）、消息已读未读状态、用户搜索（按用户名模糊搜索/手机号精准搜索）、文件与头像上传。
...
6. **WebSocket 集群实时通讯:** 基于 **WebSocket + Redis Pub/Sub** 实现跨实例消息分发，支持多实例部署下的实时聊天；结合 JWT 实现消息发送者的身份认证。
7. **房源聊天联动:** 将传统租赁平台的信息孤岛打通，租客可直接在房源详情页发起与房东的即时通讯，降低沟通成本，提升平台粘性。

## 进阶计划
目前完成的只是一些比较基础的 Java 后端的内容，还是很需要再增加深度。对于后端部分，计划增加下列功能模块：
1. ~~微聊功能~~（已实现）~~：基于 WebSocket + Redis Pub/Sub 的即时通讯~~
2. 微服务化：引入 Spring Cloud Alibaba (Nacos/OpenFeign/Sentinel) 将聊天服务、用户服务拆分为独立微服务，配合 Redis 实现分布式 WebSocket Session 管理
